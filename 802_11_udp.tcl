...
#creating nodes and saving them in an array
...
###########
puts "[expr $num_parallel_flow + $num_cross_flow + $num_random_flow  + $num_sink_flow]"
###########
...
#creating src and dest for parallel flow
#CHNG
...
#connecting src and dest (at transport layer)
#CHNG
for {set i 0} {$i < $num_parallel_flow } {incr i} {
     $ns_ connect $udp_($i) $null_($i)
}

#creating traffic generator (application layer) for src (transport layer)
#CHNG
for {set i 0} {$i < $num_parallel_flow } {incr i} {
...
#setting time for starting the flow, defualt is every parallel flow starts simultaneously,
#as $parallel_start_gap has a value 0.0
#CHNG
for {set i 0} {$i < $num_parallel_flow } {incr i} {
...
#we have already created udp agent, null agent and flow numbering from 0 upto (but not including)
#$num_parallel_flow, now we're doing the same thing from $num_parallel_flow 
#CHNG
...
#now ww've used from 0 upto k (not including)
set r $k
set rt $r
